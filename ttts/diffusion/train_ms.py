...
class Trainer(object):
    def __init__(self, args):
        ...
        self.diffusion, self.train_dataloader, self.eval_dataloader, self.optimizer, self.scheduler, self.gpt, self.dvae \
            = self.accelerator.prepare(self.diffusion, self.train_dataloader, self.eval_dataloader, self.optimizer, self.scheduler, self.gpt)
        if self.grad_clip <= 0:
            self.grad_clip = 50
        self.global_step = 0
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(self.eval_dataloader):
                ...
                latent = self.gpt(data['padded_mel_refer'], data['padded_text'],
                                  ...
                                  torch.tensor([padded_mel_code.shape[-1] * self.mel_length_compression],
                                               device=device),
